...
| --- | --- |
| `!join` | Создать манекен автора с его текущим отображаемым именем |
...
Пользователь не может указать другой `actorId` и управляет только собственным манекеном. Повторный `!join` во время создания или после успешного появления манекена игнорируется и не отправляет второй `spawn`.
