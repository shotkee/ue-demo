...
> Для нового Windows-клона рекомендуется сначала использовать корневой `setup.ps1`. Он автоматизирует установку и проверку описанных ниже компонентов, настройку TwitchBridge, генерацию файлов проекта и сборку `DemoEditor`. Эта инструкция остаётся подробным справочником для ручной установки и диагностики.
